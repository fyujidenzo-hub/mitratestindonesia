      ...
      <main className="mx-auto max-w-5xl px-4 py-6 sm:px-6 sm:py-8">
        <Card className="overflow-hidden">
          <div className="relative overflow-hidden bg-gradient-to-br from-shopee-500 to-orange-500 p-6 text-white sm:p-9">
            ...
            <div className="relative flex items-center gap-5"><div className="grid h-20 w-20 shrink-0 place-items-center rounded-3xl bg-white text-2xl font-black text-shopee-500 shadow-xl">{user?.displayName.slice(0, 2).toUpperCase()}</div><div><p className="text-xs font-black uppercase tracking-[.18em] text-white/65">Work account</p><h1 className="mt-1 text-3xl font-black">{user?.displayName}</h1><p className="mt-1 text-sm font-semibold text-white/75">@{user?.username}</p></div></div>
          </div>
          <div className="grid gap-3 p-5 sm:grid-cols-3 sm:p-7"><Stat icon={<WalletCards />} label="Balance" value={money(user?.balance ?? 0)} /><Stat icon={<Award />} label="Level" value={user?.level || "STARTER"} /><Stat icon={<ShieldCheck />} label="Tasks completed" value={String(user?.totalOrders ?? 0)} /></div>
        </Card>

        <div className="mt-5 grid gap-4 lg:grid-cols-[.72fr_1.28fr]">
          <Card className="p-5 sm:p-6"><h2 className="font-black text-slate-900">Account information</h2><p className="mt-1 text-xs font-semibold text-slate-400">Your customer account details.</p><div className="mt-5 grid gap-3"><Info icon={<UserRound />} label="Username" value={user?.username || "-"} /><Info icon={<Phone />} label="Phone number" value={user?.phone || "-"} /><Info icon={<ShieldCheck />} label="Referred by" value={user?.referrer?.displayName || "-"} /></div></Card>

          <Card className="p-5 sm:p-6">
            <div><p className="text-[10px] font-black uppercase tracking-[.16em] text-shopee-500">Everything in one place</p><h2 className="mt-1 text-xl font-black text-slate-900">Quick access</h2></div>
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              {quickLinks.map(({ to, label, hint, icon: Icon, tone }) => <Link key={to} to={to} className="group flex items-center gap-3 rounded-[22px] border border-slate-100 bg-slate-50 p-4 transition hover:-translate-y-0.5 hover:border-orange-100 hover:bg-orange-50/60"><span className={`grid h-11 w-11 shrink-0 place-items-center rounded-2xl ${tone}`}><Icon size={20} /></span><span className="min-w-0"><span className="block text-sm font-black text-slate-800 group-hover:text-shopee-600">{label}</span><span className="mt-0.5 block text-[10px] font-semibold text-slate-400">{hint}</span></span></Link>)}
            </div>
            <Button variant="ghost" className="mt-4 w-full" onClick={async () => { await logout(); navigate("/login"); }}><LogOut size={18} /> Sign out</Button>
          </Card>
        </div>
      </main>
    </CustomerShell>
