...
export default function FinancePage() {
  ...
  const withdrawalLocked = Boolean(user?.withdrawalLocked);
  const withdrawalLockMessage = withdrawalLocked
    ? user?.withdrawalRemarks?.trim() || t("Withdrawals are locked for this account.")
    : "";
  ...
  const submitWithdraw = async (event: React.FormEvent) => { event.preventDefault(); if (withdrawalLocked) return; setLoading(true); setMessage(""); try { await api("/customer/withdrawals", { method: "POST", body: JSON.stringify({ ...withdraw, amount: Number(withdraw.amount) }) }); setTone("success"); setMessage(t("Withdrawal submitted. The balance was deducted and will be refunded if the request is rejected.")); setWithdraw({ amount: "100000", bankName: "", accountName: "", accountNumber: "", withdrawalPassword: "" }); await Promise.all([load(), refresh()]); } catch (error) { setTone("error"); setMessage(t(error instanceof Error ? error.message : "Withdrawal failed.")); } finally { setLoading(false); } };
  ...
  return <CustomerShell><main className="mx-auto w-full min-w-0 max-w-6xl px-3 py-5 sm:px-6 sm:py-8"><div className="grid min-w-0 gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(320px,.75fr)] lg:gap-6"><section className="min-w-0"><p className="text-[11px] font-black uppercase tracking-[.18em] text-shopee-500 sm:text-xs">{t("Balance center")}</p><h1 className="mt-1 break-words text-2xl font-black tracking-tight text-slate-900 sm:text-3xl">{t("Manage your work balance")}</h1><p className="mt-2 text-sm font-semibold text-slate-500">{t("Current balance")} <strong className="break-all text-shopee-500">{money(user?.balance ?? 0)}</strong></p>
    ...
    {tab === "withdraw" && withdrawalLockMessage && <div role="alert" className="mt-4 flex items-start gap-3 rounded-2xl border border-rose-200 bg-rose-50 p-3.5 text-sm font-bold text-rose-700"><LockKeyhole size={19} className="mt-0.5 shrink-0" /><span className="flex-1 whitespace-pre-wrap leading-6">{withdrawalLockMessage}</span></div>}
    ...
    ) : <Card className="mt-4 min-w-0 p-4 sm:p-7">
      <div className="flex min-w-0 items-center gap-3">
        <div className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-emerald-50 text-emerald-600 sm:h-12 sm:w-12"><Wallet /></div>
        <div className="min-w-0">
          <h2 className="break-words font-black text-slate-900">{t("Request a withdrawal")}</h2>
          <p className="text-xs font-semibold leading-5 text-slate-500">{t("Minimum withdrawal Rp100,000")}</p>
        </div>
      </div>
      <form onSubmit={submitWithdraw} className="mt-5 grid min-w-0 gap-4 sm:mt-6 sm:grid-cols-2">
        <Field label={t("Withdrawal amount")}><input className={inputClass} type="number" min="100000" disabled={withdrawalLocked} value={withdraw.amount} onChange={(e) => setWithdraw({ ...withdraw, amount: e.target.value })} /></Field>
        <Field label={t("Bank name")}><input className={inputClass} required disabled={withdrawalLocked} value={withdraw.bankName} onChange={(e) => setWithdraw({ ...withdraw, bankName: e.target.value })} /></Field>
        <Field label={t("Account holder name")}><input className={inputClass} required disabled={withdrawalLocked} value={withdraw.accountName} onChange={(e) => setWithdraw({ ...withdraw, accountName: e.target.value })} /></Field>
        <Field label={t("Account number")}><input className={inputClass} required disabled={withdrawalLocked} value={withdraw.accountNumber} onChange={(e) => setWithdraw({ ...withdraw, accountNumber: e.target.value })} /></Field>
        <div className="sm:col-span-2"><Field label={t("Withdrawal PIN")}><div className="relative"><LockKeyhole size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" /><input className={`${inputClass} pl-11`} type="password" required disabled={withdrawalLocked} value={withdraw.withdrawalPassword} onChange={(e) => setWithdraw({ ...withdraw, withdrawalPassword: e.target.value })} /></div></Field></div>
        <div className="sm:col-span-2"><Button loading={loading} disabled={withdrawalLocked} className="w-full">{t("Submit withdrawal")}</Button></div>
      </form>
    </Card>}</section>
