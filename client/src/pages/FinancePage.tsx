  ...
  return <CustomerShell><main className="mx-auto w-full min-w-0 max-w-6xl px-3 py-5 sm:px-6 sm:py-8"><div className="grid min-w-0 gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(320px,.75fr)] lg:gap-6"><section className="min-w-0"><p className="text-[11px] font-black uppercase tracking-[.18em] text-shopee-500 sm:text-xs">{t("Balance center")}</p><h1 className="mt-1 break-words text-2xl font-black tracking-tight text-slate-900 sm:text-3xl">{t("Manage your work balance")}</h1><p className="mt-2 text-sm font-semibold text-slate-500">{t("Current balance")} <strong className="break-all text-shopee-500">{money(user?.balance ?? 0)}</strong></p>
    ...
    {tab === "topup" ? (
      ...
      <form onSubmit={submitWithdraw} className="mt-5 grid min-w-0 gap-4 sm:mt-6 sm:grid-cols-2">
        <Field label={t("Withdrawal amount")}><input className={inputClass} type="number" min="100000" value={withdraw.amount} onChange={(e) => setWithdraw({ ...withdraw, amount: e.target.value })} /></Field>
        <Field label={t("Bank name")}><input className={inputClass} required value={withdraw.bankName} onChange={(e) => setWithdraw({ ...withdraw, bankName: e.target.value })} /></Field>
        <Field label={t("Account holder name")}><input className={inputClass} required value={withdraw.accountName} onChange={(e) => setWithdraw({ ...withdraw, accountName: e.target.value })} /></Field>
        <Field label={t("Account number")}><input className={inputClass} required value={withdraw.accountNumber} onChange={(e) => setWithdraw({ ...withdraw, accountNumber: e.target.value })} /></Field>
        <div className="sm:col-span-2"><Field label={t("Withdrawal PIN")}><div className="relative"><LockKeyhole size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" /><input className={`${inputClass} pl-11`} type="password" required value={withdraw.withdrawalPassword} onChange={(e) => setWithdraw({ ...withdraw, withdrawalPassword: e.target.value })} /></div></Field></div>
        <div className="sm:col-span-2"><Button loading={loading} className="w-full">{t("Submit withdrawal")}</Button></div>
      </form>
    </Card>}</section>
