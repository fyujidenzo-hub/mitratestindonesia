    ...
    <CustomerShell>
      <main className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-xs font-black uppercase tracking-[.18em] text-shopee-500">{t("Rewards and activity")}</p>
            <h1 className="mt-1 text-3xl font-black tracking-tight text-slate-900">{t("Usage & transaction history")}</h1>
            <p className="mt-2 text-sm font-semibold text-slate-500">{t("Track completed tasks, automatic milestone rewards, and account transactions.")}</p>
          </div>
          <div className="rounded-2xl bg-shopee-50 px-5 py-3 text-right ring-1 ring-shopee-100">
            <p className="text-[10px] font-black uppercase tracking-wide text-shopee-500">{t("Tasks completed")}</p>
        ...
        <div className="mt-7 grid gap-6 lg:grid-cols-[.85fr_1.15fr]">
          <Card className="overflow-hidden">
            <div className="bg-gradient-to-r from-shopee-500 to-orange-500 p-5 text-white"><p className="text-xs font-black uppercase tracking-[.16em] text-white/70">Automatic bonuses</p><h2 className="mt-1 text-xl font-black">{t("Reward history")}</h2></div>
            <div className="divide-y divide-slate-100">
              {rewards.length ? rewards.map((reward) => <div key={reward.id} className="flex items-center gap-3 p-4"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-orange-50 text-shopee-500"><Gift size={19} /></span><div className="min-w-0 flex-1"><p className="break-words text-sm font-black text-slate-900">{reward.description || reward.senderName || "Task milestone reward"}</p><p className="mt-1 text-[11px] font-semibold text-slate-400">{reward.requestNumber} · {dateTime(reward.createdAt)}</p></div><p className="text-sm font-black text-shopee-600">+{money(reward.amount)}</p></div>) : <p className="p-8 text-center text-sm font-semibold text-slate-400">Complete milestone tasks to earn your first reward.</p>}
            </div>
          </Card>

          <Card className="overflow-hidden">
            <div className="bg-slate-950 p-5 text-white"><p className="text-xs font-black uppercase tracking-[.16em] text-white/50">{t("Account usage")}</p><h2 className="mt-1 text-xl font-black">{t("All transactions")}</h2></div>
            <div className="divide-y divide-slate-100">
              {transactions.length ? transactions.map((transaction) => <TransactionRow key={transaction.id} transaction={transaction} />) : <p className="p-8 text-center text-sm font-semibold text-slate-400">{t("No transactions yet.")}</p>}
            </div>
          </Card>
        </div>
      </main>
...
function TransactionRow({ transaction }: { transaction: Transaction }) {
  ...
  return <div className="flex items-center gap-3 p-4"><span className={`grid h-11 w-11 shrink-0 place-items-center rounded-2xl ${transaction.type === "REWARD" ? "bg-orange-50 text-shopee-500" : "bg-slate-100 text-slate-600"}`}><Icon size={19} /></span><div className="min-w-0 flex-1"><p className="break-words text-sm font-black text-slate-900">{transaction.description || transaction.senderName || transaction.type.replace("_", " ")}</p><p className="mt-1 text-[11px] font-semibold text-slate-400">{transaction.requestNumber} · {dateTime(transaction.createdAt)}</p></div><div className="text-right"><p className="text-sm font-black text-slate-900">{transaction.type === "WITHDRAWAL" ? "−" : "+"}{money(transaction.amount)}</p><StatusPill status={transaction.status} /></div></div>;
}
