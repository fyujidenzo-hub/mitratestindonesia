...
export function CatalogBannerCarousel({ banners = [] }: { banners?: CatalogBanner[] }) {
  ...
  return (
    ...
    >
      {slides.map((banner, index) => <figure key={banner.id} aria-hidden={index !== current} className={`absolute inset-0 overflow-hidden transition duration-700 ease-out motion-reduce:transition-none ${index === current ? "z-10 translate-x-0 opacity-100" : index < current ? "-translate-x-4 opacity-0" : "translate-x-4 opacity-0"}`}><img src={banner.imageUrl} alt="" aria-hidden="true" loading={index === 0 ? "eager" : "lazy"} decoding="async" className="absolute inset-0 h-full w-full scale-110 object-cover opacity-45" /><div className="absolute inset-0 bg-slate-950/15" /><img src={banner.imageUrl} alt={index === current ? banner.altText : ""} loading={index === 0 ? "eager" : "lazy"} fetchPriority={index === 0 ? "high" : "auto"} decoding="async" className="relative h-full w-full object-contain" /></figure>)}

      <div className="absolute bottom-3 left-1/2 z-20 flex -translate-x-1/2 items-center gap-1.5 rounded-full border border-white/15 bg-slate-950/55 px-3 py-2 shadow-lg sm:bottom-4 sm:gap-2">
      ...
      <button type="button" onClick={() => move(-1)} aria-label="Previous banner" className="absolute left-3 top-1/2 z-20 hidden h-11 w-11 -translate-y-1/2 place-items-center rounded-full border border-white/20 bg-slate-950/55 text-white shadow-lg transition hover:bg-slate-950/75 sm:grid"><ChevronLeft size={22} /></button>
      <button type="button" onClick={() => move(1)} aria-label="Next banner" className="absolute right-3 top-1/2 z-20 hidden h-11 w-11 -translate-y-1/2 place-items-center rounded-full border border-white/20 bg-slate-950/55 text-white shadow-lg transition hover:bg-slate-950/75 sm:grid"><ChevronRight size={22} /></button>
    </section>
  );
}
